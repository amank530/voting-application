...
import EciAdminDashboard from './pages/EC/EciAdminDashboard';
import VoterDashboard from './pages/voter/VoterDashboard';
import CandidateRegistration from './pages/voter/CandidateRegistration';
import ElectionsLivePage from './components/ElectionsLivePage';
import CandidateProfilesPage from './pages/voter/CandidateProfilesPage';
import RegisteredPartiesPage from './pages/party/RegisteredPartiesPage';
import AboutPage from './components/AboutPage';
import EcAdminLogin from './pages/EC/EcAdminLogin';
import DatabaseInspector from './pages/EC/DatabaseInspector';
...
export default function App() {
  ...
  const navLinks = [
    ...
    { id: 'CANDIDATE_REG', label: '📝 Become Candidate', action: handleBecomeCandidateNavigate },
    { id: 'ABOUT', label: '📢 Gazettes', action: () => { setCurrentView('ABOUT'); } },
    { id: 'DATABASE_STATE', label: '🗄️ DB Inspector', action: () => { setCurrentView('DATABASE_STATE'); } },
    { 
      id: 'ECI_ADMIN', 
      label: '⚖️ Admin', 
      action: () => { 
  ...
  const fetchBulletins = async () => {
    try {
      const apiList = await api.notifications.list();
      const voterNotifs = JSON.parse(localStorage.getItem('eci_voter_notifications') || '[]');
      const partyNotifs = JSON.parse(localStorage.getItem('eci_party_notifications') || '[]');

      const convertedVoter = voterNotifs.map(n => ({
        id: n.id || `voter-${Math.random()}`,
        title: n.title,
      ...
      const convertedParty = partyNotifs.map(n => ({
        id: n.id || `party-${Math.random()}`,
        title: n.title,
        content: n.content || n.message,
        type: n.type || 'URGENT',
        timestamp: n.timestamp || n.date || new Date().toISOString()
      ...
      const combined = [...(Array.isArray(apiList) ? apiList : []), ...convertedVoter, ...convertedParty];
      ...
      console.error('Error fetching bulletins in App.jsx:', err);
    }
  };

  useEffect(() => {
    fetchBulletins();
    const handleStorage = () => fetchBulletins();
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-between font-sans selection:bg-saffron-500 selection:text-white">
      ...
      <header className="bg-white border-b border-gray-200/80 shadow-sm sticky top-0 z-40">
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-3.5 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 relative">
            ...
            {currentUser && currentUser.role !== 'VOTER' && currentUser.role !== 'ELECTION_COMMISSION' && currentView === 'HOME' && (
                      ...
                      <div className="p-1.5 space-y-0.5 max-h-[70vh] overflow-y-auto">
                        ...
                        {!currentUser ? (
                          <button
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-gray-100 rounded-lg flex items-center gap-2.5 transition group cursor-pointer border border-gray-100"
                          >
                            <div className="w-6 h-6 rounded bg-gray-100 text-gray-700 flex items-center justify-center font-bold text-[11px] shrink-0">
                              🗳️
                            </div>
                            <div>
                              <span className="text-xs font-bold text-primary-950 block">Voter Portal</span>
                              <span className="text-[9px] text-gray-400 block">login & sign-up and Voting</span>
                            </div>
                          </button>
                        ) : (
                          <button
      ...
      <main className="flex-grow">
        <AnimatePresence mode="wait">
          ...
          ) : currentView === 'DATABASE_STATE' ? (
            <motion.div 
              key="database_state_view"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
                ...
                <>
                  ...
                  {currentUser.role === 'CANDIDATE' && (
                    <div className="max-w-xl mx-auto py-12 px-4 text-center space-y-6 animate-fade-in">
                      ...
                      <div className="pt-2">
                        <button
                          onClick={handleBecomeCandidateNavigate}
                  ...
                  {currentUser.role === 'VOTER' && (
                    <VoterDashboard 
                      ...
                      onNavigateToCandidateReg={handleBecomeCandidateNavigate}
                    />
                  )}
                </>
              ) : (
