...
import { 
  ...
  MapPin, CreditCard, CheckSquare, Users, Calendar, Mail, Phone, Globe
} from 'lucide-react';
...
export default function RegisteredPartiesPage({ currentUser, onNavigateToHome, onLoginSuccess, onLogout }) {
  ...
  const [partyForm, setPartyForm] = useState({
    ...
    motto: '',
    ideology: '',
    partyType: 'National', // (National / State / Regional / Local)
    dateOfFormation: '',
    officialEmail: '',
    officialPhone: '',
    officialWebsite: '',
    ...
    partyMembers: [
      { fullName: '', mobileNumber: '', state: '', district: '', membershipId: '', joiningDate: '' }
    ],
  ...
  const addOfficeBearer = () => {
    setPartyForm(prev => ({
      ...prev,
      officeBearers: [...prev.officeBearers, { fullName: '', position: 'Secretary', mobileNumber: '', email: '', address: '', photoUrl: '', idProofUrl: '' }]
    }));
  };
  ...
  const addPartyMember = () => {
    setPartyForm(prev => ({
      ...prev,
      partyMembers: [...prev.partyMembers, { fullName: '', mobileNumber: '', state: '', district: '', membershipId: `MEM-${Math.floor(1000 + Math.random() * 9000)}`, joiningDate: new Date().toISOString().split('T')[0] }]
    }));
  };
  ...
  const [pendingMembers, setPendingMembers] = useState([
    { id: 'pm-1', fullName: 'Vikram Joshi', mobileNumber: '9123456711', state: 'Madhya Pradesh', district: 'Bhopal', date: '2026-07-14', email: 'vikram.j@gmail.com' },
    { id: 'pm-2', fullName: 'Nisha Deshmukh', mobileNumber: '9123456712', state: 'Maharashtra', district: 'Mumbai', date: '2026-07-15', email: 'nisha.d@gmail.com' },
    { id: 'pm-3', fullName: 'Siddharth Rao', mobileNumber: '9888877766', state: 'Karnataka', district: 'Bangalore', date: '2026-07-16', email: 'sid.rao@gmail.com' }
  ]);
  ...
  const handleApproveCandidateWithLogo = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Refresh local lists
        fetchCandidates();
        // Update currently selected candidate view
        setSelectedCandidate(prev => prev ? { ...prev, status: 'APPROVED', partySymbol: allocateSymbol, partyName: activeParty.name } : null);
      }
    } catch (err) {
        ...
        {viewMode === 'BROWSE' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-gray-100 shadow-3xs flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  <Search className="w-5 h-5 text-purple-600" />
                  <div>
                    <h3 className="text-xs font-extrabold text-gray-900 uppercase">Party Search Registry</h3>
                    <p className="text-[10px] text-gray-400 font-medium">Search approved electoral parties</p>
                  </div>
                </div>
                <input 
                  type="text" 
                  ...
                  className="w-full md:w-64 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-semibold focus:bg-white focus:outline-none focus:ring-1 focus:ring-purple-500"
                />
              </div>

              <div className="bg-purple-50/60 p-4 rounded-2xl border border-purple-100 flex items-start gap-3">
                <ShieldCheck className="w-5 h-5 text-purple-700 mt-0.5 flex-shrink-0 animate-pulse" />
                <div className="text-[11px] text-purple-950 space-y-1">
                  <span className="font-extrabold uppercase block tracking-wider text-xs">Symbol Allocation Rules</span>
                  <p className="leading-relaxed">
                    National & State political party admins can allocate symbols, view candidate details, and issue certified high-command tickets directly via the login panel.
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Parties Grid */}
            {loading ? (
              <div className="bg-white py-16 text-center rounded-2xl border border-gray-100 text-gray-400 text-xs font-medium">
                Loading authorized party portfolios from ECI records...
              </div>
            ) : filteredParties.length === 0 ? (
              <div className="bg-white p-16 text-center rounded-2xl border border-gray-100 text-gray-400 text-xs italic">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredParties.map((party) => {
                  ...
                  const hasEmoji = symbolStr.includes(' ') || symbolStr.length <= 4;
                  return (
                    <div 
                      key={party.id} 
                      className="bg-white rounded-2xl border border-gray-100 shadow-3xs p-6 flex flex-col justify-between hover:border-purple-200 hover:shadow-md transition duration-200"
                    >
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4.5">
                            <div className="w-14 h-14 rounded-2xl bg-purple-50/50 border border-purple-100 text-3xl flex items-center justify-center font-bold">
                              {hasEmoji ? symbolStr : '🗳️'}
                            </div>
                            <div>
                              <h4 className="font-extrabold text-gray-950 text-sm font-display leading-tight">{party.name}</h4>
                              <span className="text-[11px] font-mono font-bold text-purple-600 uppercase tracking-wider block mt-0.5">
                                {party.abbrev} Token Registry
                              </span>
                            </div>
                          </div>
                          <span className={`text-[9px] font-bold px-2.5 py-0.5 rounded-full uppercase border tracking-wider ${
                            party.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100'
                          }`}>
                            {party.status}
                          </span>
                        </div>
                        ...
                        <div className="space-y-2 border-t border-gray-50 pt-3">
                          <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider block">Official Electoral Manifesto</span>
                          <p className="text-xs text-gray-600 leading-relaxed italic">
                            "{party.manifesto || 'No manifesto pledge filed yet. Active under verified ECI electoral token.'}"
                          </p>
                        </div>
                      </div>

                      <div className="border-t border-gray-50 mt-5 pt-3 flex justify-between text-[10px] font-mono text-gray-400">
                        <span>Registration Token: {party.id}</span>
                        <span>Audit Level: APPROVED ECI TICKET</span>
                      </div>
                    </div>
                  );
                })}
          ...
          >
            <div className="bg-white rounded-2xl border border-gray-100 shadow-xl p-8 space-y-6">
              <div className="text-center space-y-2">
                ...
                <h2 className="text-xl font-bold text-gray-900 font-display">Party Administration Access</h2>
                <p className="text-xs text-gray-400 max-w-xs mx-auto leading-normal">
                  Verify ECI-registered political party abbreviation credentials to authorize and review nominations.
                </p>
              </div>
              ...
              <form onSubmit={handlePartyLogin} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-semibold focus:bg-white focus:outline-none focus:ring-1 focus:ring-purple-600"
                    required
                  />
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition shadow-md shadow-purple-600/10 cursor-pointer"
                >
                  Verify credentials
                </button>
              </form>
              ...
              {!regSuccess && (
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                  ...
                  <form onSubmit={handleRegisterParty} className="md:col-span-9 space-y-6 text-left">
                    ...
                    {regStep === 1 && (
                      <div className="space-y-4 animate-fade-in">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div 
                            onClick={() => {
                              setPartyForm({
                                ...partyForm, 
                                logoUrl: 'party_logo_attached.png',
                                docPartyLogo: 'party_logo_attached.png'
                              });
                            }}
                          ...
                          >
                            ...
                            <span>Attach Simulated Logo File (PNG/JPG)</span>
                            {partyForm.logoUrl && (
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-gray-500 uppercase block">
                            Choose ECI Reserved Electoral Symbol *
                          </label>
                          <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5 bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                            {APPROVED_SYMBOL_TEMPLATES.map((sym, idx) => (
                              <button
                                key={idx}
                                type="button"
                                onClick={() => setPartyForm({...partyForm, symbol: sym.char})}
                                className={`p-1.5 rounded-lg border text-lg flex flex-col items-center justify-center transition cursor-pointer ${
                                  partyForm.symbol === sym.char 
                                    ? 'bg-purple-600 text-white border-purple-600 shadow-3xs scale-105 font-bold' 
                                    : 'bg-white hover:bg-gray-100 border-gray-200 text-gray-700'
                                }`}
                                title={sym.name}
                              >
                                <span>{sym.char}</span>
                                <span className="text-[6px] font-sans font-extrabold uppercase mt-0.5 text-gray-400 truncate w-full text-center">
                                  {sym.name.split(' ')[0]}
                                </span>
                              </button>
                            ))}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div 
                            onClick={() => {
                              setPartyForm({
                                ...partyForm, 
                                officeAddressProofUrl: 'office_address_proof_signed.pdf',
                                docOfficeAddressProof: 'office_address_proof.pdf'
                              });
                            }}
                            className="border border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-purple-400 cursor-pointer transition bg-gray-50/50 text-[11px] font-bold flex items-center justify-center gap-1.5 text-gray-600"
                          >
                            <Paperclip className="w-4.5 h-4.5 text-gray-400" />
                            <span>Attach Office Lease/Ownership/Utility Document Proof</span>
                    ...
                    {regStep === 3 && (
                      <div className="space-y-4 animate-fade-in">
                        <div className="border-b border-gray-100 pb-2">
                          ...
                          <p className="text-[10px] text-gray-400">Statutory credentials of the active head of the organization.</p>
                        </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div 
                              onClick={() => {
                                setPartyForm({
                                  ...partyForm, 
                                  presidentPhotoUrl: 'president_photo.jpg',
                                  docPresidentPhoto: 'president_photo.jpg'
                                });
                              }}
                            ...
                            >
                              ...
                              <span>Attach President Photo</span>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div 
                              onClick={() => {
                                setPartyForm({
                                  ...partyForm, 
                                  presidentIdProofUrl: 'president_id_proof.pdf',
                                  docPresidentIdProof: 'president_id_proof.pdf'
                                });
                              }}
                              className="border border-dashed border-gray-300 rounded-xl p-3 text-center hover:border-purple-400 cursor-pointer transition bg-gray-50/50 text-[10px] font-semibold flex items-center justify-center gap-1.5 text-gray-600"
                            >
                              <Paperclip className="w-4 h-4 text-gray-400" />
                              <span>Attach Identity Proof (Aadhaar/Passport/PAN)</span>
                    ...
                    {/* STEP 4: Office Bearers Details (Add Multiple) */}
                    {regStep === 4 && (
                      <div className="space-y-4 animate-fade-in">
                        <div className="border-b border-gray-100 pb-2 flex justify-between items-center">
                          <div>
                            <h3 className="text-xs font-bold text-purple-900 uppercase tracking-wider flex items-center gap-1.5">
                              <Users className="w-4 h-4 text-purple-600" /> Step 4: Executive Office Bearers
                            </h3>
                            <p className="text-[10px] text-gray-400">Add key high-command positions like Vice President, Treasurer, Secretary.</p>
                          </div>
                          <button
                            type="button"
                        ...
                        {partyForm.officeBearers.length === 0 ? (
                          <div className="p-8 text-center text-xs text-gray-400 border border-dashed border-gray-200 rounded-2xl italic">
                            No additional central office bearers added yet. Click "+ Add Bearer" above.
                          </div>
                        ) : (
                          <div className="space-y-4 max-h-[450px] overflow-y-auto pr-1">
                            {partyForm.officeBearers.map((bearer, idx) => (
                              <div key={idx} className="p-4 bg-gray-50/50 rounded-xl border border-gray-200 space-y-3 relative">
                                <div className="absolute right-3 top-3 flex items-center gap-2">
                                  ...
                                  {partyForm.officeBearers.length > 1 && (
                                    <button
                                      type="button"
                                      onClick={() => removeOfficeBearer(idx)}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-4">
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input 
                                      type="text" 
                                      placeholder="e.g. Vice President, National Secretary"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  <div className="space-y-1">
                                    <label className="text-[9px] font-bold text-gray-400 uppercase block">Photo</label>
                                    <button
                                      type="button"
                                      onClick={() => updateOfficeBearer(idx, 'photoUrl', 'bearer_photo.png')}
                                      className="w-full py-1.5 bg-white hover:bg-gray-100 text-gray-600 border border-gray-200 rounded-lg text-[10px] font-bold transition flex items-center justify-center gap-1"
                                    >
                                      <UploadCloud className="w-3.5 h-3.5 text-gray-400" />
                                      <span>{bearer.photoUrl ? '✓ Photo Selected' : 'Attach Photo'}</span>
                                    </button>
                                  </div>

                                  <div className="space-y-1">
                                    <label className="text-[9px] font-bold text-gray-400 uppercase block">ID Proof</label>
                                    <button
                                      type="button"
                                      onClick={() => updateOfficeBearer(idx, 'idProofUrl', 'bearer_id.pdf')}
                                      className="w-full py-1.5 bg-white hover:bg-gray-100 text-gray-600 border border-gray-200 rounded-lg text-[10px] font-bold transition flex items-center justify-center gap-1"
                                    >
                                      <Paperclip className="w-3.5 h-3.5 text-gray-400" />
                                      <span>{bearer.idProofUrl ? '✓ ID Proof Attached' : 'Attach ID Proof'}</span>
                                    </button>
                                  </div>
                                </div>
                              </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div 
                            onClick={() => {
                              setPartyForm({
                                ...partyForm, 
                                constitutionPdfUrl: 'constitution_doc_signed.pdf',
                                docPartyConstitution: 'constitution.pdf'
                              });
                            }}
                            className="border border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-purple-400 cursor-pointer transition bg-gray-50/50 text-[11px] font-bold flex items-center justify-center gap-1.5 text-gray-600"
                          >
                            <Paperclip className="w-4.5 h-4.5 text-gray-400" />
                            <span>Attach Constitution Document PDF</span>
                    ...
                    {/* STEP 6: Party Members Details (Add Multiple) */}
                    {regStep === 6 && (
                      <div className="space-y-4 animate-fade-in">
                        <div className="border-b border-gray-100 pb-2 flex justify-between items-center">
                          <div>
                            <h3 className="text-xs font-bold text-purple-900 uppercase tracking-wider flex items-center gap-1.5">
                              <FileSpreadsheet className="w-4 h-4 text-purple-600" /> Step 6: Founding Party Members
                            </h3>
                            <p className="text-[10px] text-gray-400">Directory of initial founding or verified general members.</p>
                          </div>
                          <button
                            type="button"
                        ...
                        {partyForm.partyMembers.length === 0 ? (
                          <div className="p-8 text-center text-xs text-gray-400 border border-dashed border-gray-200 rounded-2xl italic">
                            No founding members listed yet. Click "+ Add Member" to register founding body.
                          </div>
                        ) : (
                          <div className="space-y-4 max-h-[450px] overflow-y-auto pr-1">
                            {partyForm.partyMembers.map((member, idx) => (
                              <div key={idx} className="p-4 bg-gray-50/50 rounded-xl border border-gray-200 space-y-3 relative">
                                <div className="absolute right-3 top-3 flex items-center gap-2">
                                  <span className="text-[9px] font-mono bg-purple-100 text-purple-800 px-2 py-0.5 rounded-full">
                                    ID: {member.membershipId || 'MEM-AUTO'}
                                  </span>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-4">
                                  <div className="space-y-1">
                                    ...
                                    <input 
                                      ...
                                      onChange={(e) => updatePartyMember(idx, 'fullName', e.target.value)}
                                      className="w-full px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none"
                                      required
                                    />
                                  </div>
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input 
                                      ...
                                      onChange={(e) => updatePartyMember(idx, 'mobileNumber', e.target.value)}
                                      className="w-full px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none"
                                      required
                                    />
                                  </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                  <div className="space-y-1">
                                    ...
                                    <input 
                                      ...
                                      placeholder="State"
                                      value={member.state}
                                      onChange={(e) => updatePartyMember(idx, 'state', e.target.value)}
                                      className="w-full px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none"
                                    />
                                  </div>
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input 
                                      ...
                                      onChange={(e) => updatePartyMember(idx, 'district', e.target.value)}
                                      className="w-full px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none"
                                    />
                                  </div>
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <input 
                                      ...
                                      onChange={(e) => updatePartyMember(idx, 'joiningDate', e.target.value)}
                                      className="w-full px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none"
                                    />
                                  </div>
                                </div>
                    ...
                    {regStep === 8 && (
                      <div className="space-y-4 animate-fade-in">
                        <div className="border-b border-gray-100 pb-2">
                          ...
                          <p className="text-[10px] text-gray-400">Select the local, regional, state, or federal bodies your party intends to contest.</p>
                        </div>

                        <div className="bg-purple-50/50 border border-purple-100 p-4 rounded-xl text-purple-950 text-xs leading-relaxed">
                          Please select all targeted democratic bodies. The ECI Registry will configure authorization ticketing routes accordingly.
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                          {[
                            ...
                            return (
                              <label 
                                key={lvl} 
                                className={`p-3 border rounded-xl flex items-center gap-3 cursor-pointer transition select-none ${
                                  isChecked 
                    ...
                    {regStep === 9 && (
                      <div className="space-y-4 animate-fade-in">
                        <div className="border-b border-gray-100 pb-2">
                          <h3 className="text-xs font-bold text-purple-900 uppercase tracking-wider flex items-center gap-1.5">
                            <UploadCloud className="w-4 h-4 text-purple-600" /> Step 9: Consolidated Documents Checklist
                          </h3>
                          <p className="text-[10px] text-gray-400">Validate and confirm statutory proofs to avoid technical rejections.</p>
                        </div>

                        <div className="space-y-3.5">
                          {[
                            { key: 'docPartyConstitution', label: 'Party Constitution (PDF)', defaultVal: 'party_constitution_final.pdf' },
                            { key: 'docPresidentIdProof', label: 'President ID Proof (Aadhaar/PAN)', defaultVal: 'president_id_proof_final.pdf' },
                            { key: 'docPresidentPhoto', label: 'President Passport-sized Photo', defaultVal: 'president_photo_final.jpg' },
                            { key: 'docOfficeAddressProof', label: 'Registered Office Address Proof', defaultVal: 'office_address_proof_final.pdf' },
                            { key: 'docPanCard', label: 'Party Permanent Account Number (PAN) Card', defaultVal: 'party_pan_card.pdf' },
                            { key: 'docBankProof', label: 'Consolidated Bank Account Proof (Cancelled Cheque/Statement)', defaultVal: 'bank_account_proof.pdf' },
                            { key: 'docPartyLogo', label: 'Party Emblem Logo (High Resolution)', defaultVal: 'party_logo_highres.png' }
                          ].map((docItem) => {
                            ...
                            return (
                              ...
                              >
                                <div className="space-y-0.5">
                                  <h4 className="text-xs font-bold text-gray-950">{docItem.label}</h4>
                                  <p className="text-[10px] text-gray-400 font-mono">
                                    {isUploaded ? `Attached: ${partyForm[docItem.key]}` : 'Required for statutory validation node'}
                                  </p>
                                </div>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setPartyForm(prev => ({
                                      ...prev,
                                      [docItem.key]: docItem.defaultVal,
                                      // Sync other simulated state pointers if applicable
                                      ...(docItem.key === 'docPartyLogo' ? { logoUrl: docItem.defaultVal } : {}),
                                      ...(docItem.key === 'docPresidentPhoto' ? { presidentPhotoUrl: docItem.defaultVal } : {}),
                                      ...(docItem.key === 'docPresidentIdProof' ? { presidentIdProofUrl: docItem.defaultVal } : {}),
                                      ...(docItem.key === 'docOfficeAddressProof' ? { officeAddressProofUrl: docItem.defaultVal } : {}),
                                      ...(docItem.key === 'docPartyConstitution' ? { constitutionPdfUrl: docItem.defaultVal } : {})
                                    }));
                                  }}
              ...
              <div className="space-y-6 text-left animate-fade-in">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-xs space-y-4">
                    <h3 className="font-extrabold text-sm text-gray-950 font-display">Pending Membership Requests</h3>
                    <p className="text-[10px] text-gray-400 leading-relaxed">Citizens who have requested to join your party ranks online. Review and approve their admission certificates.</p>
                    ...
                    {pendingMembers.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {pendingMembers.map((req) => (
                          <div key={req.id} className="p-4 bg-gray-50 hover:bg-white hover:shadow-xs border border-gray-100 rounded-xl transition space-y-3">
                            <div className="space-y-1">
                              <h4 className="font-bold text-xs text-gray-950">{req.fullName}</h4>
                              <p className="text-[10px] text-gray-400 leading-normal">
                                From: <span className="text-gray-700 font-medium">{req.district}, {req.state}</span>
                              </p>
                              <p className="text-[10px] text-gray-400 leading-normal font-mono">Mob: {req.mobileNumber} • {req.email}</p>
                              <p className="text-[9px] font-mono text-purple-600 bg-purple-50/60 px-1.5 py-0.5 rounded inline-block">Filed: {req.date}</p>
                            </div>

                            <div className="flex gap-2">
                              <button 
                                onClick={() => handleDeclineMembershipRequest(req.id)}
                                className="flex-1 py-1 bg-white hover:bg-gray-150 border border-gray-250 text-gray-700 text-[10px] font-extrabold rounded-lg transition cursor-pointer"
                              >
                                Decline
                              </button>
                              <button 
                                onClick={() => handleApproveMembershipRequest(req.id)}
                                className="flex-1 py-1 bg-purple-600 hover:bg-purple-700 text-white text-[10px] font-extrabold rounded-lg transition cursor-pointer shadow-md shadow-purple-600/10"
                              >
                                Approve
                              </button>
                            </div>
                      ...
                      <div className="border-t border-gray-150 pt-4 space-y-3 bg-gradient-to-br from-purple-50/50 to-indigo-50/20 p-4 rounded-xl border border-purple-100/50">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 pt-1">
                          <button
                            ...
                            className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50"
                          >
                            <CheckCircle2 className="w-3.5 h-3.5" />
                            <span>{approvalActionLoading ? 'Signing LEDGER...' : 'Approve & Assign Logo'}</span>
                          </button>
                          ...
                          <button
                            ...
                            disabled={approvalActionLoading}
                            className="py-2.5 px-3 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50"
                          >
