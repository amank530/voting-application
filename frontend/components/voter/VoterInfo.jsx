...
import { Landmark, Printer, Award, ShieldCheck, Download, MapPin } from 'lucide-react';

export default function VoterInfo({ currentUser }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 flex flex-col items-center">
          ...
          <div className="flex gap-3.5 mt-4">
            <button
              onClick={handlePrint}
              className="px-4 py-1.5 bg-white hover:bg-gray-100 text-gray-700 border border-gray-200 rounded-lg text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-3xs transition"
            >
              <Printer className="w-4 h-4 text-gray-500" /> Print ID Card
            </button>
            <button
              onClick={handlePrint}
              className="px-4 py-1.5 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-sm transition"
            >
        ...
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col justify-between space-y-4">
          <div className="space-y-3 text-xs">
            <h4 className="font-bold text-gray-900 border-b border-gray-200 pb-1.5">Booth Information</h4>
            ...
            <div className="space-y-2">
              ...
              <div>
                <span className="text-[9px] font-bold text-gray-400 uppercase block">Constituency Assembly Seat</span>
                <span className="font-black text-primary-800 block">{currentUser.constituency || 'Bhopal North'}</span>
              </div>

              <div>
                <span className="text-[9px] font-bold text-gray-400 uppercase block">Allocated Booth Address</span>
                <span className="text-gray-600 block">Government Boys Higher Secondary School, Room No 1, Bhopal</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-3 rounded-lg border border-gray-200/60 text-[9.5px] text-gray-500 space-y-1">
            ...
            <p className="leading-relaxed">
              When voting in online booths, this cryptographically generated EPIC code serves as your secure ballot credential. Keep your passwords confidential.
            </p>
          </div>
        </div>
