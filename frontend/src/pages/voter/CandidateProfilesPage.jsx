...
export default function CandidateProfilesPage({ currentUser, onNavigateToHome, onNavigateToReg, onOpenAuth }) {
  ...
  // Dynamic runtime location lists aggregated from static INDIAN_REGIONS and live approved candidate profiles
  const approvedCandidates = candidates.filter(c => c.status === 'APPROVED');

  const availableStates = [...new Set([
    ...INDIAN_REGIONS.map(r => r.state),
    ...approvedCandidates.map(c => c.state).filter(Boolean)
  ])];

  const availableDistricts = selectedState 
    ? [...new Set([
        ...(INDIAN_REGIONS.find(r => r.state === selectedState)?.districts.map(d => d.name) || []),
        ...approvedCandidates.filter(c => c.state === selectedState).map(c => c.district).filter(Boolean)
      ])]
    : [];
  ...
  const availableConstituencies = selectedDistrict
    ? [...new Set([
        ...
        ...approvedCandidates.filter(c => c.state === selectedState && c.district === selectedDistrict).map(c => c.constituency).filter(Boolean)
      ])]
    : [];

  const availableCitiesGrams = selectedDistrict
    ? [...new Set([
        ...approvedCandidates.filter(c => c.state === selectedState && c.district === selectedDistrict).map(c => c.cityGramNagar).filter(Boolean)
      ])]
    : [];
